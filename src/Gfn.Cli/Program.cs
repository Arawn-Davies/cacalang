...
internal static class Program
{
    ...
    private static int Dispatch(string[] args)
    {
        ...
        {
            Console.WriteLine($"gfn {typeof(Program).Assembly.GetName().Version?.ToString(3) ?? "unknown"}");
            return ExitSuccess;
        }

        // `gfn program.gfn` keeps working as a shorthand for `gfn build`.
    ...
    {
        writer.WriteLine("""
            gfn - the Good for Nothing compiler
