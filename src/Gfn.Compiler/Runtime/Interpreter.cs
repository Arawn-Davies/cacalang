...
/// </summary>
/// <remarks>
/// This backend exists so that <c>gfn run</c> behaves identically on Windows,
...
public sealed class Interpreter
{
    ...
    /// <exception cref="GfnRuntimeException">The program failed while running.</exception>
    ...
    private object ReadValue(GfnType type)
    {
        var line = _input.ReadLine() ?? string.Empty;

        if (type == GfnType.String)
        {
            return line;
        }
        ...
        {
            throw new GfnRuntimeException($"'{line}' is not an integer");
        }
    ...
    private object EvaluateBinary(BinaryExpression binary)
    {
        ...
        switch (binary.Operator)
        {
            ...
            case BinaryOperator.Divide:
                if (b == 0)
                {
                    throw new GfnRuntimeException("attempted to divide by zero");
                }
